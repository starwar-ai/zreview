...
#### 4. 修复事件中心未定义错误
- **问题**: `wx.eventCenter is not defined` 导致定时同步功能报错
- **原因**: 事件中心的初始化在页面中，但 App 启动时就尝试调用
- **解决方案**:
  - 创建专门的 `utils/event-bus.js` 事件总线模块
  - 在 `app.js` 中使用事件总线初始化 `wx.eventCenter`
  - 在 `onLaunch()` 生命周期中初始化事件中心
  - 移除页面中重复的事件中心初始化代码
  - 添加完整的错误处理和防御性编程
  - 添加调试日志输出
